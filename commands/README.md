...
| [`/fleet-onboard`](fleet-onboard.md) | scratch | The newcomer's wizard (issue #1168, EPIC #1163): a conversation that asks which of **their own** repos and what they want done, adds the repo (`fleet-repo.sh add`), drafts the issue with them and files + spawns it (`fleet-issue-file.sh --spawn`), explains the session list with keys read live from `fleet-keys.sh`, stays with them until **they** merge the PR, then shows how to do the next one alone. Never writes code; every GitHub/fleet write is shown and confirmed first; speaks the newcomer's language; never offers the `FLEET_SEED=1` starter repo. Progress lives in `$FLEET_CONF_DIR/global/onboard.state` via [`bin/fleet-onboard.sh`](../bin/fleet-onboard.sh) (`brief` / `get` / `set` / `reset`), so calling it back resumes at the step it left. The first `brief` also leaves `global/guide.spoke` — a guide window only counts as alive once it has (issue #1215): a claude that came up but printed `Unknown command` is never recorded as `onboarded`, and the collector reopens it. |
...
- `owner: scratch` — guarded like `hub` (`$SEAT` must NOT be `worker`), but meant
  for a window of its own — a scratch the operator or the fleet opens for it
  (e.g. `/fleet-onboard`, which files and spawns issues, so it can't live inside
  a worker bound to one).
...
marker, exactly, with a single concrete owner word (`worker`, `hub`, `either`, `scratch`),
