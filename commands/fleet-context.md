...
It prints four lines — `context` (the percentage and the token count, plus which
source it came from), `session` (turns, output tokens spent, and the pre-compact
peak when the window has already been cleared), `handoff` (the thresholds in
force), and `verdict:`. Add `--json` when you want to branch on a field, or `-q`
...
in play: it reads a transcript and two tmux options and writes nothing at all.
