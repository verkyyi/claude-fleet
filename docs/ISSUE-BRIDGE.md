...
`hooks/bash-guard.py` steers a raw `gh` issue-comment from a fleet pane through
the marker-stamping wrapper (issue #483). Since #528 it **rewrites rather than
denies**: the guard returns a PreToolUse `updatedInput` that swaps just that one
statement for `fleet-comment.sh … --note`, so the rest of a compound command
still runs. It also fires only when the target issue has a **live bound worker**
— with nothing bound there is nothing to relay into, so the comment is inert and
the rail stays out of the way. (`FLEET_ALLOW_RAW_COMMENT=1`, in the environment
or inline anywhere in the command, is the hatch that keeps the call verbatim.)

Why it stopped denying: a deny kills the WHOLE Bash call. Every block this rail
produced landed on a compound command — median 1.2 KB, up to 34 statements — so
a batch that committed, probed, wrote files and only THEN posted a report lost
all of it to repair one line, and the report body had to be regenerated by hand.
Of the 43 blocks, 36 came from scratch panes with no issue binding at all and
exactly one was the pane's own issue.
...
2. **`hooks/bash-guard.py` (the hard rail)** — the bypass-perms PreToolUse hook
   (runs for **every** seat) blocks a `send-keys` segment aimed at a **fleet**
   server: the ambient `$TMUX` server when the pane is a fleet's, or `tmux -L
   <label>` where that label owns a fleet conf. It inspects only the command
   ...
   Scoped to fleet servers since #528. The rail protects a live worker's Claude
   TUI, and a server hosting no fleet has no TUI to corrupt — so an isolated test
   socket (`-S /tmp/probe.sock`, or `-L scratch` with no conf) is not guarded.
   That is the idiom CLAUDE.md prescribes for testing tmux tooling, and it was
   the collateral: 12 of the 13 blocks this rail produced were isolated-socket
   selftest harnesses, not live panes.
