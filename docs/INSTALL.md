...
| Summarizer daemon + hooks (optional) | one-line LLM summary per session → the dash summary column **and** the window's own pane header (published as a `@summary` window option the `pane-border-format` renders, issue #455 — same one call feeds both, and `window_name` is never touched); refreshed on Stop/SessionStart hooks + a ~180s catch-all daemon | `claude` CLI |
...
6. **Daemons.**
   - macOS: for each template in `launchd/`, substitute `__HOME__` with the
     ...
     **runaway-CPU watchdog** (issue #151) — no extra unit; it's OFF until a fleet
     ...
     per-session summary column and the matching `@summary` pane-header token;
     without it both just stay empty (the header falls back to index:name #issue).
   ...
   - Linux: use the ready-made units in `systemd/` (parity with the plists,
     ...
     optional
     dispatch/issue-bridge/watch/summarize/worktree-autoclean are `.timer`s too, and the
