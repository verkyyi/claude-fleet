...
| Collector daemon | git/gh/usage/issues caches every ~60s. Since issue #551: **one tick at a time** (`global/collect.pid`; a previous tick older than `FLEET_COLLECT_DEADLINE`, 600s, is killed + superseded — a tick wedged on an un-timeboxed `gh` used to block every later phase for hours) and a **per-phase heartbeat** (`global/collect.heartbeat`: phase, per-phase seconds, end/dur — `fleet-doctor.sh` prints the last tick's age, duration and slowest phase). It runs the quota watch (next row) first thing, before any gh work | gh, python3 |
| Quota-watch daemon (recommended with a ccquota hub) | `com.claude-fleet.quotawatch` (`bin/fleet-quotawatch.sh`, 60s; issue #551): the ccquota-driven **pre-emptive account rotation** (warn every session on an account at `FLEET_ACCOUNT_WARN_PCT`, bench + `migrate --account` at `FLEET_ACCOUNT_CEILING` — issue #513) on its **own** tick, decoupled from the collector's gh/git/python phases. It used to be the LAST block of the collector tick, so a slow or wedged tick starved it: on 2026-09-11 the cache went 2.5h stale, neither branch fired, and 21 sessions rode a 5h window to 100%. Now: own 60s unit + also run at the TOP of every collector tick (so an install whose daemon set predates #551 still watches at the collector's cadence); `mkdir` lock against overlap; `global/quotawatch.heartbeat`; and a **staleness alarm** — `account.quota.ts` older than `FLEET_ACCOUNT_QUOTA_STALE` (600s) with a pool + hub configured shows `⚠ quota stale 47m` on the status bar, FAILs in `fleet-doctor.sh`, and the next tick that runs notifies once (`FLEET_NOTIFY_CMD`) how long the watch was blind. `--status` / `--dry-run` for scripts and rehearsal. No-op without `FLEET_ACCOUNTS_DIR` tokens + `CCQUOTA_HUB_URL` | ccquota, python3 |
...
6. **Daemons.**
   - macOS: for each template in `launchd/`, substitute `__HOME__` with the
     ...
     cadence then rides the collector's); the **diskguard** watcher (`com.claude-fleet.diskguard`, 60s) is strongly
   ...
   - Linux: use the ready-made units in `systemd/` (parity with the plists,
     ...
     pre-emptive rotation's own 60s tick, issue #551) + the recommended
...
7. **Shell helpers.** Offer to add `source ~/.claude/fleet/shell/cw.zsh` to
   ...
   **Optional — Codex workers.** To run a fleet's workers on OpenAI Codex CLI
   ...
   once it is set, and **⌃a** on the dash flips the fleet back and forth
   (issue #554) — a `claude:` / `codex:` prefix on that line is the one-off.
