...
| Config modal (`prefix+c`) | fzf popup to view/edit `FLEET_*` config in two layers — this fleet (the login's `fleet.settings` + the fleet `conf`, one layer to you) ▸ a hosted repo's overlay — over the code default; the install's `fleet.conf` is still read as a read-only legacy layer, never written (#1102); internal pacing knobs stay under a collapsed INTERNAL "show all" header; ⌃s toggles the write scope (this fleet ⇄ repo), enter edits a key (typed validation, backup-first) | fzf ≥ 0.45 |
...
| Seed repo (new logins) | `fleet-up.sh --seed` (issue #1167): a new login's fleet comes up on a starter repo so it works from minute one, and that repo only LOOKS — the conf gets `FLEET_SEED=1` + `FLEET_AUTOFILL=0` + `FLEET_ISSUE_BRIDGE=0`, and the dispatcher + issue-bridge skip a `FLEET_SEED=1` repo whatever its switches say, so the new login never claims or relays work on a repo it can't write to (and another fleet watches). Marks the fleet conf's OWN repo only (repo-scoped; a repo added later is the login's own); `fleet-doctor` prints a `seed` INFO line. Default: unset = an ordinary repo, and a fleet-up without `--seed` writes the conf byte for byte as before | — |
...
6. **Daemons.**
   - macOS: for each template in `launchd/`, substitute `__HOME__` with the
     ...
     **records only** (never reaps a worktree) — so it is **ON by default** per
     ...
     credentials) and, when HEAD is behind it, the tree is clean, and no window on
     any of this login's fleets is working / looping / waking, fast-forwards
     `~/.claude/fleet` and runs `bin/fleet-install-apply.sh` + `bin/fleet-doctor.sh`;
     a NEW doctor FAIL rolls it back and that version is not retried until stable
   ...
   - Linux: use the ready-made units in `systemd/` (parity with the plists,
     ...
     `claude-fleet-install-sync.timer` (follow `refs/tags/stable` when idle); the
     optional
   ...
   **Ship the units as written — the scheduling class is load-bearing** (issue
   #588). Eight plists deliberately carry `ProcessType=Standard` rather than the
   `Background` the other six use: **cleanup**, **worktree-autoclean**,
   **diskguard**, **base-sync**, **dispatch**, **sleep**, **install-sync** and **collect**. `ProcessType=Background` puts
...
7. **Shell helpers.** Offer to add `source ~/.claude/fleet/shell/cw.zsh` to
   ...
   **Optional — login banner + SSH auto-attach** (issues #1068, #1166). Offer
   to add this line to `~/.zshrc`, after the `cw.zsh` line above:

   ```zsh
   # claude-fleet login: banner, then an SSH login goes straight into the fleet
   source ~/.claude/fleet/shell/fleet-login.zsh
   ```

   It prints `shell/fleet-intro.sh` — what this login's fleet holds and how to
   get in — and then, for an **interactive SSH login**, runs `cf`: attaches to
   the fleet, or starts it first when it isn't running. Detaching (`⌃b d`) drops
   you back at the shell. The gating lives in the snippet, not the scripts:
   interactive shells only (scp, rsync and `ssh host cmd` never see it), never
   inside tmux (worker panes source `.zshrc` too), and the auto-attach only with
   ...
   login into auto-attach, replace the block with the `source` line above.
...
`com.claude-fleet.base-sync`, `com.claude-fleet.install-sync`, `com.claude-fleet.dispatch`, and `com.claude-fleet.webhook`; on Linux
...
`claude-fleet-base-sync.timer` + `claude-fleet-install-sync.timer` + `claude-fleet-dispatch.timer` + `claude-fleet-webhook.service`.) If you ran the
