...
5. **Merge Claude Code hooks.** Merge `hooks/settings-hooks.json` into
   ...
   (`prompt_input_exit|logout`) fires it only on a **real** worker exit, and it is
   a **complete no-op unless a fleet sets `FLEET_CLOSE_ON_EXIT=1`**, so it is safe
   to add globally. When armed, a manual exit closes the window, gate-reaps the
