...
7. **Shell helpers.** Offer to add `source ~/.claude/fleet/shell/cw.zsh` to
   ...
   **Optional — login banner** (issue #1068). Offer to add this block to
   `~/.zshrc` so an SSH login shows what the login's fleet holds and how to get
   in. The gating lives here, not in the script: interactive shells only, never
   inside tmux (worker panes source `.zshrc` too), and `touch ~/.hushfleet` opts
   out. It replaces any hand-written per-account `~/.config/claude-fleet/intro.sh`
   — once the block points at the shipped script, delete that copy.

   ```zsh
   # claude-fleet login intro (shell/fleet-intro.sh, issue #1068)
   if [[ -o interactive ]] && [[ -z "$TMUX" ]] && [[ ! -f ~/.hushfleet ]] \
      && [[ -x ~/.claude/fleet/shell/fleet-intro.sh ]]; then
     ~/.claude/fleet/shell/fleet-intro.sh
   fi
   ```
