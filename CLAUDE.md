...
6. **Daemons.**
   - macOS: for each template in `launchd/`, substitute `__HOME__` with the
     ...
     **recommended** for every fleet, since **the fleet never merges** (issue #277):
     `/fleet-ship` arms GitHub auto-merge and this daemon reaps the leftover
     ...
     (`FLEET_CLEANUP_MAX_PER_TICK`) like the dispatcher. It **merges nothing and
...
8. **Fleet commands (optional).** Copy `commands/*.md` → `~/.claude/commands/`
   ...
   `fleet-claim`, `fleet-ship` (opens the PR + arms GitHub auto-merge — the fleet
   never merges), `fleet-blocked`, `fleet-cleanup` (manual reap of a merged/closed
