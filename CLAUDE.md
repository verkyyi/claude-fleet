...
| Auto-triage (optional) | Inline LLM pass on the one-line capture (issue #235): with `FLEET_AUTO_TRIAGE=1`, `⌃n`/`prefix+n` runs `bin/fleet-triage.sh` (one `claude -p`, model `FLEET_AUTO_TRIAGE_MODEL`=haiku) to expand the line into a refined title + elaborated body + component milestone + type label + `priority:pN` — every milestone/label VALIDATED against the repo's real sets (hallucinations dropped, never invented; control labels stripped). NOT a daemon — single-writer by the capture process, no lease/disk-gate. OFF by default; spends LLM tokens per capture. See docs/AUTO-TRIAGE.md | gh + claude |
...
| Cross-machine pre-spawn dedup | every spawn (`bin/dash-issue-session.sh`, the one choke point) consults the shared GitHub issue as a claim ledger before spawning, so two fleets on **different machines / same repo** don't both spawn `issue-<N>` (duplicate worktrees + push race + competing PRs) — the local tmux dedup only sees one machine. Taken (assignee · non-open state · `▶ claiming` comment · open PR) ⇒ **refuse**; free ⇒ **claim AT SPAWN** (assignee + marker, not on the worker's first `/fleet-claim` turn — that gap was the race) then a best-effort **tie-break** (earliest `▶ claiming` REST-comment-id wins) rolls back the just-created window/worktree/branch and refuses if a peer claimed in the same sub-second overlap. **NOT a mutex** (GitHub has no CAS on an issue) — it shrinks the race window, doesn't eliminate it. `--force`/`--reclaim` spawns past a stale claim; scouts are exempt (read-only, no push/PR). **ON by default** — the cost is a few gh reads/spawn (claim-at-spawn just moves `/fleet-claim`'s writes earlier; a gh outage degrades to spawn-anyway) and it self-disables when gh is absent; a single-machine fleet wanting the zero-gh fast path sets `FLEET_PRESPAWN_DEDUP=0`. `/fleet-claim` stays but no-ops when it finds the pre-claim | gh (authed) |
...
| Self-land (optional) | Worker owns its FULL lifecycle incl. the land (`commands/fleet-land-self.md` + `bin/fleet-land-self.sh`): the worker merges its OWN PR — per-repo land-lease-serialized (shared `bin/fleet-land-lease.sh`, hold-through-green, steal-if-stale, `--match-head-commit`), base fast-forward, self-destruct. Two gates via `FLEET_SELF_LAND`: **`=1` (triggered)** — after `/fleet-ship` the worker waits; the steward triggers by commenting `/land` on the issue (relayed by the issue-bridge) and only *approves* instead of *performing* the merge (implies `FLEET_ISSUE_BRIDGE=1`). **`=auto` (issue #270)** — the trigger becomes OPTIONAL: `/fleet-ship` flows straight into `/fleet-land-self` (still waits for CI green), NO `/land` required, needs no bridge; CI-green + branch protection become the only gate (same relaxation class as `FLEET_AUTOLAND`). **Relaxes the "workers never self-merge" rail** — re-gated by the trigger (`=1`) or by CI + branch protection (`=auto`). OFF by default (`FLEET_SELF_LAND=1\|auto` per fleet); spends LLM tokens. See docs/SELF-LAND.md | gh (+ issue-bridge for `=1`) |
...
| Auto-land (optional) | `com.claude-fleet.land` (~60s): **hands-off lands landable-green PRs** (issue #233) by driving the same mechanical `bin/fleet-land.sh` the dash `⌃l`/`/fleet-land` use — no steward turn, no human. Detection is cache-only: it reads the `prmap_<slug>` the pr-refresh daemon already writes (ZERO extra `gh`) and lands only OPEN PRs whose `ready` column is exactly `ready` (green + mergeable now; `behind`/`conflict`/`blocked` left to the steward). Single-writer per repo + disk-gated + rate-limited (`FLEET_AUTOLAND_MAX_PER_TICK`, default 1) like the dispatcher; the shared land-lease inside `fleet-land.sh` serializes it with every other lander, which re-validates against live `gh` and EJECTS (never force-merges) a PR that drifted. **Deliberately RELAXES the human approval gate — CI-green + branch protection become the ONLY gate**, so bound it with `FLEET_AUTOLAND_LABEL` (only issues carrying it auto-land; fail-closed). `--dry-run` prints intent without landing. OFF by default (`FLEET_AUTOLAND=1` per fleet); the daemon spends no tokens itself but each land runs `fleet-land`'s merge. See docs/AUTOLAND.md | gh |
...
6. **Daemons.**
   - macOS: for each template in `launchd/`, substitute `__HOME__` with the
     ...
     **docs/ISSUE-BRIDGE.md**.
     ...
     land (`com.claude-fleet.land`, 60s) is the **auto-land daemon** — install it
     only if a fleet sets `FLEET_AUTOLAND=1`. It hands-off lands landable-green PRs
     (prmap `ready` rows) by driving `bin/fleet-land.sh`, single-writer + disk-gated
     + rate-limited (`FLEET_AUTOLAND_MAX_PER_TICK`) like the dispatcher. It
     **deliberately REMOVES the human approval gate** (CI-green + branch protection
     become the only gate), the same class of relaxation as self-land/autofill — so
     it is OFF by default; ask before installing, mention that it lands with no human
     in the loop, and recommend bounding it with `FLEET_AUTOLAND_LABEL` (only issues
     carrying that label auto-land). The daemon spends no tokens itself; each land
     runs `fleet-land`'s merge. `--dry-run` prints intent without landing. Full
     design in **docs/AUTOLAND.md**.
   - Linux: use the ready-made units in `systemd/` (parity with the plists,
     ...
     dispatch/issue-bridge/watch/land/classify/summarize/worktree-autoclean are `.timer`s too.
