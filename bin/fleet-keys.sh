...
print_sheet() {
  ...
  key "→ / ←" "unfold / fold the highlighted row's subtree. A session spawned from another one nests under it (↳ tag + └ indent), and those children are COLLAPSED BY DEFAULT — the parent row's \`3/5 ✓ · 1!\` badge is what the folded block says, so the list stays one line per parent. → opens the block you are on, ← shuts the block you are IN (from the parent row or from any child in it, which puts the cursor back on the parent). A child in \`needs\` NEVER folds away — any of the three red glyphs (\`?\` question · \`⊘\` permission · \`!\`) — because the quiet layer folds and the loud one does not. ▸ / ▾ on a row marks a folded / open block. The closed view (⌃t) nests and folds the same way, off the ledger's own record of what spawned what. With text typed on the prompt line, ←/→ are that line's cursor keys as always"
