...
print_sheet() {
  ...
  key "$(dg answer)" "answer the highlighted worker's AskUserQuestion — a tappable list per question, one tap each (issue #605). This is the ONLY way to answer one: a SendMessage is delivered between turns, and a pending question IS the turn, so the message waits for the answer that waits for the message. Answers only an AskUserQuestion — a \`needs\` row parked on a PERMISSION prompt is left alone (that one you press yourself), and nothing is typed unless the chosen option is visibly on the worker's screen$(dn answer)"
