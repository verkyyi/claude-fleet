...
[ "$(printf '%s\n' "$rows" | grep -c .)" -eq 11 ] || fail "A: expected 11 dash actions" "$rows"
...
[ "$(km C-b '' actions | grep -c .)" -eq 11 ] || fail "A: actions lists 11 names"
...
[ "$(printf '%s\n' "$rows" | awk '$6=="ok"' | grep -c .)" -eq 10 ] || fail "C: the other 10 actions stay put" "$rows"
