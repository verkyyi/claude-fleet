...
[ "$(printf '%s\n' "$rows" | grep -c .)" -eq 12 ] || fail "A: expected 12 dash actions" "$rows"
...
[ "$(km C-b '' actions | grep -c .)" -eq 12 ] || fail "A: actions lists 12 names"
...
ok "A stock C-b: 12 actions at their defaults, agent = ctrl-v/⌃v, clean, env evals"
...
[ "$(printf '%s\n' "$rows" | awk '$6=="ok"' | grep -c .)" -eq 11 ] || fail "C: the other 11 actions stay put" "$rows"
