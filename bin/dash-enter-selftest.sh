...
run_enter() {
  ...
  DISPLAY_LOG="$DISPLAY_LOG" SPAWN_LOG="$SPAWN_LOG" \
    bash "$WORK/bin/dash-enter.sh" "${TARGET:-sess:1}" "$1" ${SEL+"$SEL"} >"$WORK/out" 2>"$WORK/err"
}
...
# E. the highlighted row rides to the spawn (issues #997/#1010): the bind's
# `{2}:{4}` — a session row's window id, or a heading's spawn target — arrives as
# arg 3 and becomes --selection, which dash-raw-session.sh resolves
# (fleet_selection_repo). Arg 1 (`sess:idx`, the jump target) never rides: the
# resolver reads `@<wid>`, and #1010 was the binds sending `{1}` there. A value
# outside the id/repo charset is dropped — it is embedded in a command string.
reset_debounce; : > "$SPAWN_LOG"
TARGET='hdr' SEL='hdr:o/beta' GUARD_MS=1000 GUARD_SLEEP=0.2 run_enter 'on beta'
wait_bg 0.5
grep -qF -- "--selection='hdr:o/beta'" "$SPAWN_LOG" 2>/dev/null \
  || grep -qF -- "--selection=hdr:o/beta" "$SPAWN_LOG" || fail "E a heading's {2}:{4} rides as --selection=hdr:<repo>" "$(cat "$SPAWN_LOG")"
reset_debounce; : > "$SPAWN_LOG"
TARGET='sess:3' SEL='@7:' GUARD_MS=1000 GUARD_SLEEP=0.2 run_enter 'on a row'
wait_bg 0.5
grep -qF -- "--selection=@7:" "$SPAWN_LOG" || fail "E a session row rides as --selection=@<wid>: — its {2} (#1010)" "$(cat "$SPAWN_LOG")"
grep -qF -- "sess:3" "$SPAWN_LOG" && fail "E the sess:idx jump target must never ride as the selection (#1010)" "$(cat "$SPAWN_LOG")"
reset_debounce; : > "$SPAWN_LOG"
TARGET='sess:3' SEL="x';touch $WORK/pwned;'" GUARD_MS=1000 GUARD_SLEEP=0.2 run_enter 'hostile'
wait_bg 0.5
[ -e "$WORK/pwned" ] && fail "E a hostile selection must never reach the shell"
grep -qF 'SPAWN hostile' "$SPAWN_LOG" || fail "E a hostile selection still spawns (today's rule)" "$(cat "$SPAWN_LOG")"
grep -qE -- "--selection=[^ ]" "$SPAWN_LOG" && fail "E a hostile selection is dropped, not forwarded" "$(cat "$SPAWN_LOG")"
ok "E the highlighted row rides to the spawn as --selection={2}:{4}; a hostile value is dropped (#997/#1010)"
