...
#   • kill-window @sibling from a HUB pane (@hub=1) → ALLOWED (issue #177:
#                              the operator's hub kills a merged worker's window).
#   • kill-window @sibling from a HUB pane while a NON-hub window is the
#                              ACTIVE one → still ALLOWED (#177 reopen: the seat
#                              is read from the caller's $TMUX_PANE, not the
#                              focused pane).
#   • kill-window @sibling with an EMPTY $TMUX_PANE, hub window ACTIVE →
#                              REFUSED (#177 reopen: never guess the seat from the
#                              active pane; empty caller pane is conservatively
#                              NOT the hub).
#   • kill-window @sibling with FLEET_HUB=1 in the env + EMPTY $TMUX_PANE →
#                              ALLOWED (issue #202: the durable inherited seat
#                              marker is the primary signal, so a hub shell is
...
# --- HUB-pane cross-window kill is ALLOWED (issue #177) --------------------
# The operator's hub pane (@hub=1) legitimately kills a
# merged worker's window as the last step of a cleanup reap. Build a hub window
# (hp, marked) + a worker window (wk), then kill wk FROM the hub pane — a
# cross-window kill the guard must ALLOW because the caller is the hub.
tmux new-window -t t: -n hp 2>/dev/null || fail "could not create hub-pane window hp"
tmux new-window -t t: -n wk   2>/dev/null || fail "could not create worker window wk"
PANE_HUB="$(pane_of hp)"; PANE_WK="$(pane_of wk)"; WK="$(win_id wk)"
[ -n "$PANE_HUB" ] && [ -n "$PANE_WK" ] && [ -n "$WK" ] || fail "could not build hub/worker fixture"
tmux set-option -p -t "$PANE_HUB" @hub 1 2>/dev/null || fail "could not mark hub pane"
guard "$PANE_HUB" kill-window -t "$WK" 2>/dev/null \
  || fail "hub-pane kill-window @sibling should be ALLOWED (#177)"
win_gone wk || fail "worker window wk should be gone after a hub-pane kill-window"

# --- a WORKER-seat cross-window kill is STILL refused (the #158 guarantee) ------
# Same shape, but the caller pane has NO @hub marker → must be refused again.
# Caller = a fresh, unmarked worker pane; target = the (marked) hub window,
# a genuine cross-window kill. The guard keys off the CALLER's seat, so it must
# refuse — and the hub window survives.
tmux new-window -t t: -n wk 2>/dev/null || fail "could not recreate worker window wk"
PANE_WK="$(pane_of wk)"; HUBW="$(win_id hp)"
[ -n "$PANE_WK" ] && [ -n "$HUBW" ] || fail "could not build worker-seat refusal fixture"
if guard "$PANE_WK" kill-window -t "$HUBW" 2>/dev/null; then
  fail "worker-seat kill-window @sibling must STILL be REFUSED (#158 must hold)"
fi
win_gone hp && fail "hub-pane window hp must survive a refused worker-seat kill-window"

# --- HUB exemption is FOCUS-INDEPENDENT (issue #177 reopen) ----------------
# The reopen: the pre-fix guard read the seat from `display-message -t ""`, which
# falls back to the *active* pane. The plan hub has two panes — dash (no @hub)
# and the hub pane (@hub=1). With the dash focused, the hub's own cleanup-reap
# was misread as a worker and REFUSED. The fix resolves the seat STRICTLY from the
# caller's $TMUX_PANE. Prove it: focus a NON-hub window as the active pane,
# then land FROM the hub pane — the active pane isn't the caller, yet it must
# still be ALLOWED.
tmux new-window -t t: -n hp2 2>/dev/null || fail "could not create hub window hp2"
tmux new-window -t t: -n wk2   2>/dev/null || fail "could not create worker window wk2"
PANE_HUB2="$(pane_of hp2)"; WK2="$(win_id wk2)"
[ -n "$PANE_HUB2" ] && [ -n "$WK2" ] || fail "could not build focus-independence fixture"
tmux set-option -p -t "$PANE_HUB2" @hub 1 2>/dev/null || fail "could not mark hub pane hp2"
tmux select-window -t t:wk2 2>/dev/null   # focus a NON-hub window as the active pane
guard "$PANE_HUB2" kill-window -t "$WK2" 2>/dev/null \
  || fail "hub-pane kill must be ALLOWED with a non-hub window focused (#177 reopen)"
win_gone wk2 || fail "worker window wk2 should be gone after a focus-independent hub kill"
...
# a hub window happened to be focused it would WRONGLY allow a cross-window
# kill from a caller with no resolvable pane. The fix treats an empty caller pane
# as conservatively NOT the hub. Mark + FOCUS a hub window, then call with an
# EMPTY TMUX_PANE targeting a sibling worker → must be REFUSED, worker survives.
tmux new-window -t t: -n hp3 2>/dev/null || fail "could not create hub window hp3"
tmux new-window -t t: -n wk3   2>/dev/null || fail "could not create worker window wk3"
PANE_HUB3="$(pane_of hp3)"; WK3="$(win_id wk3)"
[ -n "$PANE_HUB3" ] && [ -n "$WK3" ] || fail "could not build empty-TMUX_PANE fixture"
tmux set-option -p -t "$PANE_HUB3" @hub 1 2>/dev/null || fail "could not mark hub pane hp3"
tmux select-window -t t:hp3 2>/dev/null   # hub window is now the ACTIVE pane
if guard "" kill-window -t "$WK3" 2>/dev/null; then
  fail "empty \$TMUX_PANE must be REFUSED even with a hub window active (no active-pane guess)"
fi
win_gone wk3 && fail "worker window wk3 must survive a refused empty-\$TMUX_PANE kill"

# --- FLEET_HUB=1 env is the PRIMARY seat signal, even with EMPTY $TMUX_PANE
# (issue #202) --------------------------------------------------------------------
# #185's strict $TMUX_PANE read intermittently saw an empty caller pane and refused
# the hub's own cleanup-reap kill-window (the reopen this issue fixes). The fix
# gives the hub a durable inherited env marker (FLEET_HUB=1, exported by
# hub-session.sh) that the guard trusts FIRST — so a hub Bash-tool shell
# whose $TMUX_PANE didn't get re-exported is STILL allowed. Prove it: no pane
# marker, empty $TMUX_PANE, but FLEET_HUB=1 in the env → ALLOWED. (This
...
tmux new-window -t t: -n hp4 2>/dev/null || fail "could not create hub window hp4"
...
# hp4 (not wk4). Otherwise a freshly-created target IS the active window and the
# self-teardown path would allow the kill regardless of seat — masking the test.
tmux select-window -t t:hp4 2>/dev/null
FLEET_HUB=1 guard "" kill-window -t "$WK4" 2>/dev/null \
  || fail "FLEET_HUB=1 must ALLOW kill-window @sibling even with an empty \$TMUX_PANE (#202)"
win_gone wk4 || fail "worker window wk4 should be gone after a FLEET_HUB=1 kill (#202)"
