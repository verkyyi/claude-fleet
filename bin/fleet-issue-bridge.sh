...
# the single durable/auditable channel for driving a worker — operator→worker,
# worker→worker, external-collaborator→worker — replacing flaky `tmux send-keys`
# handbacks.
#
...
# Only the UNIT's own mode counts: the webhook and fleet-comment.sh drive the
# DELIVER path directly, and crediting those would keep this stamp fresh while
# the `--poll` backstop unit was never scheduled.
...
case " $* " in *' --poll '*) [ -f "$BIN/fleet-daemon-lib.sh" ] && { . "$BIN/fleet-daemon-lib.sh"
...
# render, a capture-pane quirk) the channel would defer FOREVER, silently — a
# handback the operator is waiting on would never land, with no signal. So bound it:
# after this many consecutive typing-defers of the SAME comment,
...
# dedup set, one file per fleet (capped so it can't grow without bound), routed
# through bridge_state_file so it lands in the #181 per-fleet layout.
...
# The bounded half-typed-input gate (issue #191 + #195). rc 3 = KEEP DEFERRING
...
bridge_input_busy() {
  python3 "$BIN/fleet-input.py" --socket "$1" --pane "$2" --busy
}
...
# Per repo: the repo-wide comment stream, with one watermark (bridge_<slug>.since)
# + one dedup seen-set, under a single per-repo lease.
poll_repo() {
