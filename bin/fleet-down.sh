...
# teardown — disarm crash auto-restore (fleet-restore.sh) so the watcher doesn't
# resurrect it. A real crash never runs fleet-down, so it stays armed and gets
# restored on the next watcher tick / login.
if ! tmux info >/dev/null 2>&1; then
  ...
  # then refresh sessmap so the dead session drops out immediately.
